...
/* int64_t bmp_take_readings_callback(alarm_id_t id) */
{
  ...
  /* uint16_t _current_eeprom_address = eeprom_get_four_bytes(&eeprom,
...
int64_t ssd1306_rtc_generic_callback(alarm_id_t id, void *arg) {
  ...
  case 3:
    ...
    temp_time = localtime(&temp_time_t);
    // printf("%s\tfrom %d seconds ago,", buf, (approx_epoch(&t) -
    // bmp_readings.low_temp_time));
  ...
  case 6:
    // printf("\n[case 6]\t");
    /* bmp280_read_raw(&raw_temperature, &raw_pressure); */
    /* pressure = bmp280_convert_pressure(raw_pressure, raw_temperature,
     * &params); */
